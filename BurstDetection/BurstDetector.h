...
class ScreenLayer
{
	...
	int insert(uint64_t flow_id)
	{
		...
		else
		{
			...
			{
				int z = bobhash[i]->run((char *)&flow_id, 8) % size;
				//if(rand()%((int) ((pow(exp, alpha - counter[z]) + 1) / 0.9)))  // sigmoid
					counter[z]--;
				if(counter[z] == 0)
					id[z] = 0;
			}
...
class Log
{
		...
		for(int i = 0; i < size; i++)
			for(int j = 0; j < bucket_size; j++)
			{
				...
				if(counter[flag][i][j] <= counter[flag ^ 1][i][j] / 2 && timestamp[i][j] != -1)
				{
					...
					//printf("%d %d %d\n",timestamp[i][j], time, id[i][j]); 
					//printf("A %I64u-window burst! ID=%I64u\n", time - timestamp[i] - 1, id[i]);
					timestamp[i][j] = -1;
				}
	...
	bool insert(uint64_t flow_id, uint32_t flow_time, uint32_t flow_count)
	{
		...
		if(flow_count > mi)
		{
			if(id[z][l] != 0 && timestamp[z][l] != -1 && counter[flag][z][l] * 2 <= counter[flag^1][z][l]) // check burst
				Record.push_back(Burst(timestamp[z][l], flow_time, id[z][l]));
			id[z][l] = flow_id;
			timestamp[z][l] = -1;
			counter[flag][z][l] = flow_count; // evict a flow!
			return true;
